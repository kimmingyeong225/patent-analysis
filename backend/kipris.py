...
def fetch_patent_data_from_kipris(query: str):
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"KIPRIS API Error (Status {response.status_code}): {response.text}")
            return []
        ...
        print(f"KIPRIS API Request Exception: {e}")
        return []
        ...
        for idx, item in enumerate(items):
            ...
            mapped_item = {
                ...
                "공개등록공보": {
                    ...
                    "publication_date": item.get('OpeningDate') or "",
                    "registration_date": item.get('RegistrationDate'),
                    "abstract": item.get('Abstract') or "요약이 제공되지 않았습니다.",
                    "claims": ["청구범위 정보는 상세조회 API에서 확인 가능합니다."],
                    "doc_type": item.get('RegistrationStatus') or "공개"
                },
                "인용문헌": {
                    "cited_by_count": 0,
                    "citing_count": 0,
                ...
                "법적상태": {
                    "status": item.get('RegistrationStatus') or "상태 알 수 없음",
                    "status_code": "R0000",
                    "last_event": item.get('RegistrationStatus') or "",
                    "last_event_date": item.get('OpeningDate') or "",
