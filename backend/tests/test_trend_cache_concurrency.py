...
#
# 검증 대상: cache._get_or_build_trend_cache
...
def cache_module():
    ...
    _trend_cache / _trend_key_locks 모두 비운다.
    """
    if "cache" in sys.modules:
        del sys.modules["cache"]
    import cache
    cache._trend_cache.clear()
    with cache._trend_cache_guard:
        cache._trend_key_locks.clear()
    yield cache
    cache._trend_cache.clear()
    with cache._trend_cache_guard:
        cache._trend_key_locks.clear()
...
def test_same_key_concurrent_requests_build_once(cache_module):
    ...
    def worker():
        return cache_module._get_or_build_trend_cache("same-query", slow_build)
    ...
    # 캐시 저장 확인
    assert cache_module._trend_cache["same-query"] == _fake_trend_result("shared")
...
def test_different_keys_build_in_parallel(cache_module):
    ...
    def worker(tag: str):
        return cache_module._get_or_build_trend_cache(f"query-{tag}", make_build(tag))
...
def test_build_failure_does_not_poison_cache(cache_module):
    ...
    with pytest.raises(RuntimeError, match="simulated KIPRIS timeout"):
        cache_module._get_or_build_trend_cache("will-fail", failing_build)

    # 캐시에 실패값 저장 안 됨
    assert "will-fail" not in cache_module._trend_cache
    ...
    result = cache_module._get_or_build_trend_cache("will-fail", succeeding_build)
    assert result == _fake_trend_result("recovered")
    assert attempts["count"] == 2
    assert cache_module._trend_cache["will-fail"] == _fake_trend_result("recovered")


def test_cache_hit_skips_build(cache_module):
    """이미 캐시에 있는 query 는 build_fn 호출 없이 바로 반환."""
    cache_module._trend_cache["preloaded"] = _fake_trend_result("preloaded")

    build_fn = MagicMock()
    result = cache_module._get_or_build_trend_cache("preloaded", build_fn)
...
def test_build_returning_none_is_not_cached(cache_module):
    ...
    r1 = cache_module._get_or_build_trend_cache("empty-trend", empty_build)
    r2 = cache_module._get_or_build_trend_cache("empty-trend", empty_build)

    assert r1 is None
    assert r2 is None
    assert "empty-trend" not in cache_module._trend_cache
